...
class HttpManager {
    ...
    async netFetch(url, method = 'GET', params, json, header, text) {
        ...
        if (method !== 'GET') {
            const now = +new Date();
            let requestData = {
                params: aesUtil.utils.encrypt(params, true),
                timeStamp: now,
                sign: aesUtil.utils.getSign(now, params),
            };

            if (json) {
                requestParams = this.formParamsJson(method, requestData, headers);
            } else {
                requestParams = this.formParams(method, requestData, headers);
            }
        } else {
